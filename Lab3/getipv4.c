/* Count the number of IPv4 packets and the byte size during each minute*/
...
static void per_packet(libtrace_packet_t *packet)
{
	...
	ipv4 = trace_get_ip(packet);

	/* Get the timestamp for the current packet */
	ts = trace_get_timeval(packet);
	
	/* If the packet does not have a IPv4 header, skip it */
	if (!ipv4)
		return;
	...
	if (next_report == 0) {
		...
		printf("TIME\t\tIPv4 PACKETS\tBYTES\n");
	}
